...
DogStatsD는 DataDog에서 statsd 프로토콜을 자체적으로 구현한 커스텀 메트릭 수집 시스템이다. statsd 프로토콜 구현 뿐만 아니라 DataDog만의 추가 기능을 확장하였다.

![image](https://github.com/user-attachments/assets/914f3116-d6c3-437d-942e-ac6b11443958)

실제 메트릭 수집 프로세스는 스프링 컨테이너가 주기적으로 사이드카 패턴으로 떠있는 데이터독 에이전트 컨테이너로 actuator 메트릭을 송신하고, 데이터독 에이전트 컨테이너는 데이터독 중앙서버에 수집된 메트릭을 송신하게 된다.
