...
# 사내에서 프라이빗 서브넷에 존재하는 RDS 에 직접 접근할 수 있는 이유는?
- 프라이빗 서브넷에 존재하는 RDS 의 경우엔 외부 인터넷에 직접 접근이 불가능하다.
- 하지만 `AWS Site-to-Site VPN` 서비스를 사용하면 IPSec 암호화 프로토콜을 사용하여 사내 온디맨드 네트워크망과 VPC Cloud 환경을 연결해줄수 있다.
  - 간단하게 설명하면 아래 이미지와 같이 AWS에는 VGW(Virtual Private Gateway)가 있고 On-Premise에는 CGW(Customer Gateway)가 붙어있으며 이 둘 사이에 IPSec 프로토콜을 이용해 터널링을 만들어 줘 인터넷을 통해서 상호 간 통신이 가능하게끔 해주는 원리다.
