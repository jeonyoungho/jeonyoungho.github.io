...
더 나아가 왜 `/favicon.ico` 리소스 요청이 발생했을까를 고민해보면 인증서버에서 로그인 성공 후의 redirect 처리로 인해 api 서버의 callback url로 요청이 들어오게 되면서 발생하게된게 아닐까 싶다. 브라우저의 기본동작으로 웹 페이지를 로드시, 탭에 표시될 아이콘(파비콘)을 자동으로 요청하는데 명시적으로 설정된 파비콘이 없을 경우 기본적으로 `/favicon.ico` 경로로 요청을 보내기 때문인것 같다.